...
def _bytes_to_unicode(val, encoding):
    if hasattr(val, 'decode'):
        return val.decode(encoding)
    return val

def _unicode_to_bytes(val, encoding):
...
class CoerceUnicode(sa_types.TypeDecorator):
    ...
    def process_bind_param(self, value, dialect):
        return _bytes_to_unicode(value, dialect.encoding)
...
class BaseReflector(object):
    ...
    def normalize_name(self, name):
        name = _bytes_to_unicode(name, self.dialect.encoding)
        if name is not None:
    ...
    def denormalize_name(self, name):
        ...
        if not self.dialect.supports_unicode_binds:
            name = _unicode_to_bytes(name, self.dialect.encoding)
        else:
            name = unicode(name)
        return name
    ...
    def _get_default_schema_name(self, connection):
        ...
        """
        default_schema_name = connection.execute('SELECT CURRENT_SCHEMA FROM SYSIBM.SYSDUMMY1').scalar()
...
class DB2Reflector(BaseReflector):
    ...
    def has_table(self, connection, table_name, schema=None):
        ...
        select = sql.select([self.sys_tables.c.tabname], whereclause)
    ...
    def has_sequence(self, connection, sequence_name, schema=None):
        ...
        select = sql.select([self.sys_sequences.c.seqname], whereclause)
        cursor = connection.execute(select)
        return cursor.first() is not None
        ...
        for row in connection.execute(query):
            ...
            if coltype in ('DECIMAL', 'NUMERIC'):
                ...
                try:
                    ...
                    util.warn("Did not recognize type '%s' of column '%s'" % (coltype, row[0]))
                    coltype = sa_types.NULLTYPE

            sa_columns.append({'name': self.normalize_name(row[0]),
                               'type': coltype,
                               'nullable': row[3] == 'Y',
                               'default': row[2] or None,
                               'autoincrement': (row[6] == 'Y') and (row[7] != ' ')})
        return sa_columns


    @reflection.cache
    def get_primary_keys(self, connection, table_name, schema=None, **kw):
        return self.get_pk_constraint(connection, table_name, schema, **kw)['constrained_columns']
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        for row in connection.execute(query):
            if row[0] not in fschema:
                ...
                fschema[row[0]] = {'name': self.normalize_name(row[0]),
                ...
                fschema[row[0]]['constrained_columns'].append(self.normalize_name(row[3]))
    ...
    def get_incoming_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        query = sql.select([sysfkeys.c.fkname, sysfkeys.c.fktabschema,
                            ...
                            sysfkeys.c.pkname, sysfkeys.c.pktabschema,
                            sysfkeys.c.pktabname, sysfkeys.c.pkcolname],
                           sql.and_(sysfkeys.c.pktabschema == current_schema,
        ...
        for row in connection.execute(query):
            if row[0] not in fschema:
                constrained_schema = self.normalize_name(row[1])
                ...
                fschema[row[0]] = {'name': self.normalize_name(row[0]),
                                   ...
                                   'constrained_table': self.normalize_name(row[2]),
                                   'constrained_columns': [self.normalize_name(row[3])],
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        query = sql.select([sysidx.c.indname, sysidx.c.colnames, sysidx.c.uniquerule, sysidx.c.system_required],
                           sql.and_(sysidx.c.tabschema == current_schema,
        ...
        for row in connection.execute(query):
            if row[2] != 'P':
                if row[2] == 'U' and row[3] != 0:
                    continue
                indexes.append({'name': self.normalize_name(row[0]),
                                'column_names': [self.normalize_name(col)
                                                 for col in col_finder.findall(row[1])],
                                'unique': row[2] == 'U'})
        return indexes
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        ...
        query = sql.select([syskeycol.c.constname, syskeycol.c.colname],
                           sql.and_(syskeycol.c.constname == sysconst.c.constname,
        ...
        for row in connection.execute(query):
            if curr_constraint == row[0]:
                unique_constraints[-1]['column_names'].append(self.normalize_name(row[1]))
            else:
                curr_constraint = row[0]
                unique_constraints.append({'name': self.normalize_name(curr_constraint),
                                           'column_names': [self.normalize_name(row[1])]})
...
class AS400Reflector(BaseReflector):
    ...
    def has_table(self, connection, table_name, schema=None):
        ...
        if current_schema:
            ...
            whereclause = self.sys_tables.c.tabname == table_name
        select = sql.select([self.sys_tables], whereclause)
        cursor = connection.execute(select)
        return cursor.first() is not None
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        query = sql.select([syskeyconst.c.colname, sysconst.c.tabname],
                           sql.and_(syskeyconst.c.conschema == sysconst.c.conschema,
        ...
        return {'constrained_columns': [self.normalize_name(key[0]) for key in connection.execute(query)],
                'name': None}
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        query = sql.select([sysidx.c.indname, sysidx.c.uniquerule, syskey.c.colname],
                           sql.and_(syskey.c.indschema == sysidx.c.indschema,
        ...
        for row in connection.execute(query):
            ...
            if key in indexes:
                ...
                indexes[key] = {'name': self.normalize_name(row[0]),
