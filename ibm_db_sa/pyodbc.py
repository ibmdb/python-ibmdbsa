...
class DB2Dialect_pyodbc(PyODBCConnector, DB2Dialect):
    ...
    def create_connect_args(self, url):
        ...
        logger.debug("Starting create_connect_args for DB2Dialect_pyodbc")
        ...
        for param in ('ansi', 'unicode_results', 'autocommit'):
            if param in keys:
                logger.debug("Setting connect_arg %s=%s", param, keys[param])
                connect_args[param] = util.asbool(keys.pop(param))
        if 'odbc_connect' in keys:
            logger.debug("Using provided ODBC connection string")
            ...
            if dsn_connection:
                ...
                if 'port' in keys and not 'port' in query:
                    ...
                    logger.debug("Detected port: %s", port)
                database = keys.pop('database', '')
                logger.debug("Host: %s", keys.get('host'))
            ...
            if user:
                logger.debug("User provided for connection")
            ...
            if 'odbc_autotranslate' in keys:
                logger.debug("Setting odbc_autotranslate option")
                connectors.append(
        ...
        logger.debug("Constructed ODBC connection string: %s", safe_conn_str)
...
#    pyodbc_driver_name = "iSeries Access ODBC Driver"
    ...
    def create_connect_args(self, url):
        url, ibmdbsa_log_value = init_ibmdbsa_logging(url)
        logger.info("entry create_connect_args()")
        logger.debug("Starting create_connect_args for AS400Dialect_pyodbc")
        opts = url.translate_connect_args(username='user')
        opts.update(url.query)
        keys = opts
        query = url.query
        ...
        if 'odbc_connect' in keys:
            ...
            if dsn_connection:
                ...
                logger.debug("Using IBM i Access ODBC driver")
                connectors = [
                    "DRIVER={%s}" % keys.pop('driver', self.pyodbc_driver_name),
            ...
            if 'odbc_autotranslate' in keys:
                logger.debug("Setting AutoTranslate option")
                connectors.append(
                    "AutoTranslate=%s" % keys.pop("odbc_autotranslate")
                )
            connectors.extend(['%s=%s' % (k, v) for k, v in keys.items()])
        conn_str = ";".join(connectors)
        safe_conn_str = re.sub(r'PWD=[^;]*', 'PWD=****', conn_str, flags=re.IGNORECASE)
        logger.debug("Constructed AS400 ODBC connection string: %s", safe_conn_str)
        logger.info("exit create_connect_args()")
        return [[";".join(connectors)], connect_args]
